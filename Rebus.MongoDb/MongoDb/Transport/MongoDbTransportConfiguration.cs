...
class MongoDbTransportConfiguration
{
    /// <summary>
    ...
    /// operations, constrained within one <see cref="MongoDbMessageProducer"/> or <see cref="MongoDbMessageConsumer"/> instance)
    /// </summary>
    ...
    /// </summary>
    public MongoDbMessageProducer CreateProducer() => new(this);

    /// <summary>
    /// Creates a consumer using the current configuration and the given <paramref name="queueName"/>
    /// </summary>
    public MongoDbMessageConsumer CreateConsumer(string queueName) => new(this, queueName);
